...
const colors = ref(['from-primary to-deep-blue', 'from-deep-blue to-blue-400', 'from-primary/80 to-primary', 'from-gray-700 to-black']);
...
<template>
    <section class="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" id="event">
        <div class="flex items-center justify-between mb-12">
            <div>
                ...
                <p class="text-gray-500 dark:text-gray-400">Mantente conectado con nuestras actividades semanales.
                </p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <mid-card v-for="(event, index) in filterList" :key="event.title" :event="event.title" :icon="getEmoji(event.icon)"
                :location="event.location" :tag="event.tag" :date="event.start.toLocaleString('es-Es', {
                    weekday: 'short', // Día abreviado (lun.)
                    month: 'short',   // Mes abreviado (feb.)
                    day: 'numeric',   // Día del mes (22)
                    year: 'numeric',  // Año (2026)
                    hour: '2-digit',  // Hora (15)
                    minute: '2-digit',// Minutos (30)
                    hour12: false     // Formato 24h
                }).toLocaleUpperCase()" :colors="colors[index]" />
        </div>
    </section>
    ...
    <modal :show="isModalOpen" title="Eventos" size="w-3xl" @close="handleClose">
        <div class="flex flex-col overflow-hidden">
            <div class="relative">
                <vue-cal v-model:events="eventos" :editable-events="false" view="days"
