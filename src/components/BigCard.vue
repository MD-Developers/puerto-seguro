...
function copy(){
    navigator.clipboard.writeText(`${props.bank}\n${props.phone}\n${props.cedula}\nConcepto: ${props.title}`);
}
</script>
<template>
    <div
        ...
        <div class="w-20 h-20 rounded-2xl bg-primary/10 text-secondary flex items-center justify-center mb-6">
        ...
        <p class="text-gray-500 dark:text-gray-400 mb-8 grow">{{ description }}</p>
        <button @click="openModal"
            class="w-full py-4 bg-primary text-background-dark font-bold rounded-xl hover:scale-[1.02] transition-transform cursor-pointer">Donar
            Ahora</button>
    </div>
        ...
        <div class="w-full flex justify-center items-center">
            <div
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <p class="text-white/50 text-xs uppercase tracking-widest font-bold mb-1">Número
                            de Telefono</p>
                        <p class="text-lg font-semibold">{{ phone }}</p>
                    </div>
                    <div>
                        <p class="text-white/50 text-xs uppercase tracking-widest font-bold mb-1">Número de Cedula</p>
                        <p class="text-lg font-semibold">{{ cedula }}</p>
                    </div>
                    <div class="pt-2">
                        <button @click="copy" class="flex items-center gap-2 text-primary font-bold text-sm cursor-pointer active:scale-95 transition-all">
