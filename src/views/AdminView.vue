    ...
    http.get('/events')
        .then((response) => {
            eventos.value = response.data.map((evento) => {
                return {
                    ...evento,
                    end: new Date(evento.end_datetime),
                    start: new Date(evento.start_datetime)
                }
            });
        }).catch((error) => {
            if (typeof error.response.data.detail === 'string') {
...
<template>
    ...
    <modal :show="isOpen" title="Agregar Eventos" @close="handleClose">
        <div
            class="flex mb-5 mt-5 relative bg-white/10 p-5 flex-wrap gap-1 justify-end transition-all duration-300 ease-in-out">
            <input v-model="newEvent.title" type="text" placeholder="Titulo"
            ...
            <div class="mt-2 flex gap-1">
                <button @click="cancelEvent"
                    class="bg-info/10 hover:bg-info/20 backdrop-blur-md text-white border border-white/30 px-3 py-1 rounded-sm font-bold text-lg transition-all cursor-pointer">Cancelar</button>
                <button @click="saveEvent"
                    class="bg-primary hover:bg-primary/90 text-white px-3 py-1 rounded-sm font-bold text-lg shadow-xl transition-all flex items-center gap-2 cursor-pointer">Guardar</button>
            </div>
        </div>
    </modal>
    <modal :show="modalConfirm" title="¿Seguro que desea borrar el evento?" @close="handleCloseConfirm">
        <div class="flex flex-col items-center">
            <p class="text-white/50 text-sm">Se borrara el evento registrado.</p>
            <div class="flex justify-center items-center gap-3 mt-2">
                <button
                    class="bg-info/10 hover:bg-info/20 backdrop-blur-md text-white border border-white/30 px-3 py-1 rounded-sm font-bold text-lg transition-all cursor-pointer flex items-center gap-2"
